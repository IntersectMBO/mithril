...
- **UNSTABLE**:
  ...
  - Support for reading the protocol configuration from on-chain markers in the Mithril aggregator and signer:
    - The nodes read the protocol parameters and the signed entity types configuration from the Cardano chain with the new `protocol_configuration_reader_adapter_config` configuration parameter.
    - New `protocol-configuration export-markers` and `protocol-configuration import-markers` commands in the Mithril aggregator to export the active configuration and to generate the datum written on chain.
    - **REMOVED** the `protocol_parameters`, `cardano_transactions_signing_config` and `cardano_blocks_transactions_signing_config` parameters from the Mithril aggregator configuration.
  ...
  - Preliminary support for the circuit verification key registry, a genesis-signed whitelist (with revocations) of the circuit verification keys trusted for SNARK certificates.
    ...
    - The registry is retrieved over HTTPS and the last verified registry is kept when a refresh fails.
