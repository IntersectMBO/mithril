...
#[derive(Default)]
pub struct DummyProtocolConfigurationMarkersReader {
    markers: RwLock<ConfigurationComputerFromMarkers>,
}

impl DummyProtocolConfigurationMarkersReader {
...
#[async_trait]
impl ProtocolConfigurationMarkersReader for DummyProtocolConfigurationMarkersReader {
...
mod tests {
    ...
    async fn empty_dummy_reader() {
        let reader = DummyProtocolConfigurationMarkersReader::default();

        let result = reader.read().await.expect("dummy reader shall not fail reading");
    ...
    async fn dummy_reader_output() {
        ...
        assert_eq!(
            markers,
            reader.read().await.expect("dummy reader shall not fail reading")
        );
    }
}
