...
impl MarkersMithrilNetworkConfigurationProvider {
    ...
    fn log_inconsistency_error(&self, epoch: Epoch, err: &InconsistentSignedEntityConfigError) {
        warn!(
            &self.logger, "Some allowed signed entity could not be enabled; using only the usable subset";
            "epoch" => %epoch,
            "error" => %err
        );
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn get_network_configuration_retrieve_configurations_for_aggregation_next_aggregation_and_registration()
     {
    ...
    #[tokio::test]
    async fn get_network_configuration_removes_unavailable_discriminants_when_config_missing() {
