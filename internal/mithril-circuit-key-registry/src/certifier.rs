...
pub struct CachedCircuitVerificationKeyCertifier {
    /// Certifier obtaining and verifying the registry.
...
impl CachedCircuitVerificationKeyCertifier {
    ...
    pub fn new(certifier: MithrilCircuitVerificationKeyCertifier, logger: Logger) -> Self {
        Self {
            certifier,
    ...
    async fn get_verified_registry(&self) -> StdResult<CircuitVerificationKeyRegistry> {
        ...
        let refreshed = self.certifier.get_verified_registry().await;
        match cache.as_mut() {
