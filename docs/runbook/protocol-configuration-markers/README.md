...
> [!IMPORTANT]
> if its the first time on the targeted environment that has no protocol configurations on chain, add `--default` at the end of the protocol-configuration export-markers command

```bash
./mithril-aggregator protocol-configuration export-markers --target-path $ASSETS_PATH/protocol-configurations.json
```

Default JSON prettified export will look like bellow :
