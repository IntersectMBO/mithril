...
- The `gcloud` CLI authenticated on the GCP project of the Mithril infrastructure, with the
  following roles:
  - `roles/compute.instanceAdmin.v1` to create the machine and bind the CI role on it
  - `roles/iam.serviceAccountAdmin`, `roles/iam.roleAdmin` and `roles/iam.serviceAccountKeyAdmin`
    to create the service account for the CI, its custom role and its key
- The `iam.disableServiceAccountKeyCreation` organization policy not enforced on the project, which
  is required to create the key of the service account for the CI
...
Create a machine sized for the workload, with an Ubuntu 24.04 image and no service account, so
the jobs running on it cannot obtain GCP credentials from the metadata server:
...
about one hour), and export it (a machine already registered refuses a new registration: follow
the `Remove the runner` section first, except the machine deletion):
...
Once connected, copy the removal token from the `Remove runner` dialog of the
[runners page](https://github.com/IntersectMBO/mithril/settings/actions/runners) of the
repository, and export it:

```bash
export REMOVAL_TOKEN=**REMOVAL_TOKEN**
```

Stop and uninstall the runner service, then unregister the runner:
...
- Self-hosted runners of a public repository must never be exposed to code from fork pull
  ...
  triggers.
- A fork pull request can still target the runner label from its own copy of a workflow, so set
  the `Approval for running fork pull request workflows from contributors` option (in
  `Settings > Actions > General`) to `Require approval for all external contributors`, and never
  approve a run that modifies a workflow to target the runner label.
- The machine has no service account, so a job cannot use the metadata server to act on the GCP
  project: only the CI service account below can start and stop it.
