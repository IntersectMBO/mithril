...
impl ClosedKeyRegistration {
    ...
    /// Compute a mapping between the signer index in the concatenation merkle tree
    /// and the one in the snark merkle tree.
    #[cfg(feature = "future_snark")]
    pub(crate) fn compute_concatenation_to_snark_index_mapping(
        &self,
    ) -> HashMap<SignerIndex, SignerIndex> {
        let mut index_map = HashMap::new();
        let mut snark_index: SignerIndex = 0;
        for (index, entry) in self.closed_registration_entries.iter().enumerate() {
            if entry.get_verification_key_for_snark().is_some() {
                index_map.insert(index as SignerIndex, snark_index);
                snark_index += 1;
            }
        }
        index_map
...
mod tests {
    ...
    #[cfg(feature = "future_snark")]
    fn snark_entry_at_stake(
        stake: Stake,
        schnorr_vk: SchnorrVerificationKey,
        rng: &mut ChaCha20Rng,
    ) -> RegistrationEntry {
        RegistrationEntry::new(
            VerificationKeyProofOfPossessionForConcatenation::from(&BlsSigningKey::generate(rng)),
            stake,
            Some(schnorr_vk),
        )
        .unwrap()
    ...
    #[cfg(feature = "future_snark")]
    #[test]
    fn compute_indices_mapping_maps_concatenation_indices_to_local_snark_positions() {
        let mut rng = ChaCha20Rng::from_seed([0u8; 32]);
        ...
        let mut concat_only_init = Initializer::new(params, 1, &mut rng);
        concat_only_init.strip_snark_keys();
        kr.register_by_entry(&concat_only_init.try_into().unwrap()).unwrap();

        let shared_schnorr_vk =
            SchnorrVerificationKey::new_from_signing_key(SchnorrSigningKey::generate(&mut rng));
        let entry_x = snark_entry_at_stake(5, shared_schnorr_vk, &mut rng);
        let entry_y = snark_entry_at_stake(10, shared_schnorr_vk, &mut rng);
        kr.register_by_entry(&entry_x).unwrap();
        kr.register_by_entry(&entry_y).unwrap();

        let z_schnorr_vk =
            SchnorrVerificationKey::new_from_signing_key(SchnorrSigningKey::generate(&mut rng));
        let entry_z = snark_entry_at_stake(15, z_schnorr_vk, &mut rng);
        kr.register_by_entry(&entry_z).unwrap();

        let closed_reg = kr.close_registration(&params).unwrap();

        assert_eq!(
            HashMap::from([(1, 0), (3, 1)]),
            closed_reg.compute_concatenation_to_snark_index_mapping(),
            "only X (concatenation 1) and Z (concatenation 3) hold SNARK keys; A (born concat-only) and Y \
             (dedup-stripped) should be absent from the mapping entirely"
        );
    }
