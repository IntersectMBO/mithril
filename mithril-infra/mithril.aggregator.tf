...
resource "null_resource" "mithril_aggregator" {
  ...
  triggers = {
    ...
    mithril_circuit_verification_key_registry_url                                = var.mithril_circuit_verification_key_registry_url,
...
EOT
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "export CIRCUIT_VERIFICATION_KEY_REGISTRY_URL='${var.mithril_circuit_verification_key_registry_url}'",
