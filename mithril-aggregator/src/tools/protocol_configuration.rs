...
impl ProtocolConfigurationTools {
    ...
    pub fn verify_configurations_against_chain(
        ...
        let epoch_range_to_verify = current_epoch.0.saturating_sub(EPOCH_OFFSET)..=current_epoch.0;
        info!(
            &self.logger,
            "Verifying configurations for epoch range [{:?}]", epoch_range_to_verify
        );
        ...
        for epoch in epoch_range_to_verify.map(Epoch) {
            ...
            if marker_on_chain.is_some() {
                ...
                warn!(
                    &self.logger,
                    "No configuration found on chain for {}, this is normal behavior if there is no protocol configuration marker on chain",
                    epoch
                );
            }
        }
