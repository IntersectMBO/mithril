...
pub trait ConfigurationSource {
    ...
    /// aggregate signature type requires it, none when no registry source is configured.
...
pub struct ServeCommandConfiguration {
    ...
    ///
    /// A `file://` URL reads the registry from a local file (tests and local deployments).
...
impl ServeCommandConfiguration {
    ...
    pub fn new_sample(tmp_path: PathBuf) -> Self {
        ...
        Self {
            ...
            protocol_configuration_reader_adapter_config: Some(AdapterConfig::Fake {
...
impl ConfigurationSource for ServeCommandConfiguration {
    ...
    fn circuit_verification_key_registry_url(&self) -> Option<String> {
        self.circuit_verification_key_registry_url.clone()
    }
