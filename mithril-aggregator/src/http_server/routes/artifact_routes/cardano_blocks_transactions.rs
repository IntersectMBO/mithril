...
fn artifact_cardano_blocks_transactions(
    router_state: &RouterState,
) -> impl Filter<Extract = (impl warp::Reply + use<>,), Error = warp::Rejection> + Clone + use<> {
    warp::path!("artifact" / "cardano-blocks-transactions")
        .and(warp::get())
...
pub mod handlers {
    ...
    /// List Cardano Blocks Transactions set artifacts
    pub async fn list_artifacts(
            ...
            .await
        {
            ...
            Err(err) => {
                warn!(logger, "list_artifacts_cardano_blocks_transactions"; "error" => ?err);
...
pub mod tests {
    ...
    async fn test_cardano_blocks_transactions_get_ok() {
        ...
        let path = "/artifact/cardano-blocks-transactions";
