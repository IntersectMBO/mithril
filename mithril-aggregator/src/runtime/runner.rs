...
pub mod tests {
    ...
    async fn build_runner_with_fixture_data(
        ...
        let fixture_builder = MithrilFixtureBuilder::default().with_signers(5);
        #[cfg(feature = "future_snark")]
        let current_fixture = fixture_builder
            .build_at_epoch(current_epoch.offset_to_signer_retrieval_epoch().unwrap());
        let next_fixture = fixture_builder.build_at_epoch(current_epoch);
        #[cfg(feature = "future_snark")]
        deps.init_state_from_fixtures(&current_fixture, &next_fixture, current_epoch)
            .await;
        #[cfg(not(feature = "future_snark"))]
        deps.init_state_from_fixture(&next_fixture, current_epoch).await;
    ...
    async fn test_inform_new_epoch() {
        ...
        deps.certifier_service = Arc::new(mock_certifier_service);
        let fixture = MithrilFixtureBuilder::default().build_at_epoch(current_epoch);
