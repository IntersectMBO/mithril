...
async fn certificate_chain() {
    ...
    let fixture_at_epoch_3 = MithrilFixtureBuilder::default()
        .with_signers(5)
        .with_protocol_parameters(protocol_parameters.clone())
    ...
    cycle!(tester, "idle");
    cycle!(tester, "ready");
    let fixture_at_epoch_5 = MithrilFixtureBuilder::default()
        .with_signers(updated_stake_distribution.len())
        .with_protocol_parameters(protocol_parameters.clone())
        .with_stake_distribution(StakeDistributionGenerationMethod::Custom(
            updated_stake_distribution.clone(),
        ))
        .build_at_epoch(Epoch(5));
    tester
    ...
    cycle!(tester, "idle");
    cycle!(tester, "ready");
    let fixture_at_epoch_6 = MithrilFixtureBuilder::default()
        .with_signers(updated_stake_distribution.len())
        .with_protocol_parameters(protocol_parameters.clone())
        .with_stake_distribution(StakeDistributionGenerationMethod::Custom(
            updated_stake_distribution,
        ))
        .build_at_epoch(Epoch(6));
    tester
